...
class GaussianDiffusion:
    ...
    def p_sample_loop_gt_prefix(
        ...
        for i, sample in enumerate(self.p_sample_loop_progressive(    # 10-step denoising
            model,
            shape,
            noise=noise,
            clip_denoised=clip_denoised,
            ...
            if dump_steps is not None and i in dump_steps:
                dump.append(deepcopy(sample["sample"]))
            final = sample  # [bs, njoints, nfeats, pred_len]

        if dump_steps is not None:
            return dump

        # if 'prefix' in model_kwargs.get('y', {}).keys() and not is_first_prefix:
        #     final["sample"] = torch.cat([model_kwargs['y']['prefix'], final["sample"]], dim=-1)
        
        return final["sample"]  # [bs, njoints, nfeats, pred_len]
    ...
    def p_sample_loop_progressive(
        ...
        # 10 denoising steps: [9, 8, 7, 6, 5, 4, 3, 2, 1, 0]
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, self._scale_timesteps(t), **model_kwargs) # NOTE: model.forward(), [bs, njoints, nfeats, pred_len]
